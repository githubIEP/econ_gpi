...
all <- all %>% 
  mutate(geocode = ifelse(country == "Kosovo", "XKO", geocode),
         country = ifelse(geocode== "CZE", "Czechia", country)) %>% rename(iep_country = country, 
                                                                           iep_geocode = geocode,
                                                                           iep_region = gpi_region)
...
all = left_join(gti_country_names,all, by= "iep_geocode")
...
all = all %>%
  rename(iep_country = iep_country.x,
         iep_region=iep_region.x) %>%
